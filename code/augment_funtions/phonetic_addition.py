...
class PhoneticAddition:
    def __init__(self):
        ...
        # Initial consonant to final consonant mapping
        self.INITIAL_TO_FINAL_MAPPING = {
            # consonants that can be directly obtained
            'ㄱ': 'ㄱ', 'ㄴ': 'ㄴ', 'ㄷ': 'ㄷ', 'ㄹ': 'ㄹ', 
            'ㅁ': 'ㅁ', 'ㅂ': 'ㅂ', 'ㅅ': 'ㅅ', 'ㅇ': 'ㅇ',
            
            # transformations according to pronunciation rules
            'ㅈ': 'ㄷ',  # ㅈ -> ㄷ
            'ㅊ': 'ㄷ',  # ㅊ -> ㄷ  
            'ㅌ': 'ㄷ',  # ㅌ -> ㄷ
            'ㅍ': 'ㅂ',  # ㅍ -> ㅂ
        ...
        # consonants that can be extracted from final consonant (for initial consonant addition)
        self.FINAL_TO_INITIAL_MAPPING = {
            # extract initial consonant from single final consonant
            'ㄱ': 'ㄱ',    # 각이 -> 각기
            'ㄴ': 'ㄴ',    # 간이 -> 간니
            'ㄷ': 'ㄷ',    # 낫이 -> 낟디  
            'ㄹ': 'ㄹ',    # 물이 -> 물리
            ...
            # extract initial consonant from double final consonant (use back consonant)
            'ㄶ': 'ㅎ',    # 않을 -> 안헐 (ㅎ 활용)
            'ㅀ': 'ㅎ',    # 싫어 -> 실허 (ㅎ 활용)
            'ㄳ': 'ㅅ',    # 몫이 -> 목시 (ㅅ 활용)
            'ㄵ': 'ㅈ',    # 앉아 -> 안자 (ㅈ 활용)
...
if __name__ == "__main__":
    ...
    print("=== Comprehensive test of Korean phonetic addition module ===\n")